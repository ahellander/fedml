...
class KerasSequentialCifar(BaseLearner):
    ...
    def partial_fit(self, x, y, classes=None, data_set_index=0, data_order=None):
        """ Do a partial fit. """
        ...
        if training_steps is not None:
            ...
            while end_ind > x.shape[0]:
                ...
                ind.append(data_order[np.arange(start_ind, x.shape[0])])
                data_order = np.random.permutation(x.shape[0])

            ind.append(data_order[np.arange(start_ind,end_ind)])
